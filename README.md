...
--Fetching Length from T_periodic table from the average of last 5 minut of end of the calio  this code added by jitu on 11/06/25
     ...
     select  ROUND(NVL( MAX( ACT_LENGTH),0),2)+7 into V_ACT_LENGTH
...
--End of code to fetch lengh
        ...
        IF DAUGHTER_COIL IS NULL THEN ---no. 3
         then DAUGHTER_COIL:=MOTHER_COIL;
...
--- End length code-- 
--Added by jitu on 17/11/25 -- Here I am fetching  SO coating 
SELECT
    ...
    CASE 
        ...
        WHEN SUBSTR(CGP_CD_COAT, 1, 1) = 'A' THEN
            ...
            END
    END AS LN_AZ_COAT into V_SO_COAT

FROM T_GP2_PDI where CGP_ID_COIL=MOTHER_COIL order by CGP_TOC desc;
...
---End
   ...
   -- V_NET_WEIGHT := ((V_INPUT_THICKNESS * ( V_ACT_WIDTH +4) * 7.85 *  (V_ACT_LENGTH+6))/1000000);

 V_NET_WEIGHT := ((V_INPUT_THICKNESS * V_ACT_WIDTH *V_ACT_LENGTH* 7.85 +  (V_ACT_WIDTH*V_ACT_LENGTH*V_SO_COAT)/1000000)/1000000;
